...
public class StatisticsAdapter extends SqliteAdapter {
	...
	public int[] getMonthlyAdded() {
		...
		try {
			...
			cursor = db.rawQuery(query, new String[] { Integer.toString(m_calendar.get(Calendar.YEAR)) });
	...
	public int[] getDailyReviewed() {
		...
		try {
			db = getDatabase();
			query = "SELECT " + " CAST(CAST(SUBSTR(LastReviewed, 1, 4) AS INTEGER) AS TEXT) as Year, "
			...
			cursor = db.rawQuery(query, new String[] { strYear, strMonth });
	...
	public Statistics getStatitstics() {
		...
		try {
			...
			query = "SELECT "
					...
					+ "SELECT *, LENGTH(Word) AS WordLength FROM Words ORDER BY WordLength DESC LIMIT 1" + ") Longest,"
