...
public class MemoView implements IInject {
	...
	public void injcet(View view, ResourceManager resources) {
		...
		holder.m_language.setText(m_memo.getWordA().getLanguage().getCode() + "\n" + m_memo.getWordB().getLanguage().getCode());
		...
		if (!m_memo.getActive()) {
			...
			holder.m_language.setPaintFlags(holder.m_language.getPaintFlags() | Paint.STRIKE_THRU_TEXT_FLAG);
		} else {
			holder.m_original.setPaintFlags(holder.m_original.getPaintFlags() & ~Paint.STRIKE_THRU_TEXT_FLAG);
			holder.m_translate.setPaintFlags(holder.m_translate.getPaintFlags() & ~Paint.STRIKE_THRU_TEXT_FLAG);
			holder.m_language.setPaintFlags(holder.m_language.getPaintFlags() & ~Paint.STRIKE_THRU_TEXT_FLAG);
	...
	private static class ViewHolder {
		...
		public ViewHolder(View view, ResourceManager resources) {
			...
			m_language = (TextView)view.findViewById(R.id.memolist_listview_txtLanguages);


			Typeface thinFont = resources.getThinFont();
			resources.setFont(m_original, thinFont);
			resources.setFont(m_translate, thinFont);
			resources.setFont(m_language, thinFont);
