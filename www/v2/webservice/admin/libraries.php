...
class PageAdapter extends DbAdapter {
	...
	public function getLibraries() {
		$db = parent::connect();
		$stm = $db->query("SELECT P.PublishedMemoBaseId AS Id, M.Name AS Name FROM memoling_PublishedMemoBases AS P INNER JOIN memoling_MemoBases AS M ON P.MemoBaseId = M.MemoBaseId");
		$stm->execute();
	...
	public function getLanguages() {
		$db = parent::connect();
		$stm = $db->query("SELECT DISTINCT LanguageIso639 FROM memoling_Words ORDER BY LanguageIso639");
		$stm->execute();
	...
	public function getUsers() {
		$db = parent::connect();
		$stm = $db->query("SELECT COUNT(*) FROM memoling_FacebookUsers");
		$stm->execute();
		$row = $stm->fetch();
		return $row[0];
	}
	...
	public function getWords() {
		$db = parent::connect();
		$stm = $db->query("SELECT COUNT(*) FROM memoling_Words");
		$stm->execute();
		$row = $stm->fetch();
		return $row[0];
	}
	...
	public function getDownloads() {
		$db = parent::connect();
		$stm = $db->query("SELECT SUM(Downloads) FROM memoling_PublishedMemoBases");
		$stm->execute();
		$row = $stm->fetch();
	...
	public function getLinks() {
		$db = parent::connect();
		$stm = $db->query("SELECT Name, Link FROM memoling_FacebookUsers ORDER BY Name");
		$stm->execute();
